		...
		size_t numTargetPoints)
{
	...
	if ( kgrid[0]*kgrid[1]*kgrid[2] != energies.size() )
	...
	for (size_t i = 0 ; i < energies.size(); ++i)
		floatArray->InsertValue(i,energies[i]);
	...
	kfWeights_ = std::vector<double>(decimator->GetOutput()->GetNumberOfPoints(), 0.0);
	for(vtkIdType i = 0; i < decimator->GetOutput()->GetNumberOfCells(); i++)
	{
		...
		for (size_t j=0; j < 3; j++)
			kfWeights_[ pointIdsCell->GetId(j) ] += area/3.0;
	}
}

size_t FermiSurface::get_npts() const
{
