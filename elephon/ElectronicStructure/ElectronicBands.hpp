		...
		std::vector<T> * hessianFieldPtr ) const
{
	...
	auto const & d = grid_.get_grid_dim();
	...
	for ( int ipz = -1; ipz <= 1; ++ipz)
		for ( int ipy = -1; ipy <= 1; ++ipy)
			for ( int ipx = -1; ipx <= 1; ++ipx)
			{
				...
				grid_.get_lattice().reci_direct_to_cartesian_2pibya(k);
	...
	if ( gradientFieldPtr  != nullptr)
		gradientFieldPtr->resize(bandIndices.size()*reducibleKPTIndices.size()*3);
	if ( hessianFieldPtr  != nullptr)
		hessianFieldPtr->resize(bandIndices.size()*reducibleKPTIndices.size()*6);

	for ( int ibm = 0 ; ibm < bandIndices.size(); ++ibm)
		for ( int ikrm = 0 ; ikrm < reducibleKPTIndices.size(); ++ikrm)
		{
			int ib = bandIndices[ibm];
			int ikr = reducibleKPTIndices[ikrm];
			auto xyz = grid_.get_reducible_to_xyz(ikr);
			auto xyzMod = xyz;
			int n = 0;
			for ( int ipz = -1; ipz <= 1; ++ipz)
				for ( int ipy = -1; ipy <= 1; ++ipy)
					for ( int ipx = -1; ipx <= 1; ++ipx)
					{
						...
						int ikrn = grid_.get_xyz_to_reducible(xyzMod);
						bval[n] = (*this)(grid_.get_maps_red_to_irreducible()[ikrn], ib);
						n++;
					}
			...
			int offset = ikrm*bandIndices.size()+ibm;
			if ( gradientFieldPtr  != nullptr)
			{
