...
BOOST_AUTO_TEST_CASE( Linear_Interpolation_Exact )
{
	...
	std::vector<size_t> testGrid = {2,2,2};

	elephon::Algorithms::TrilinearInterpolation triLin(testGrid);
	...
	for ( size_t i = 0 ; i < testGrid[0]; ++i)
		for ( size_t j = 0 ; j < testGrid[1]; ++j)
			for ( size_t k = 0 ; k < testGrid[2]; ++k)
				data[(i*testGrid[1]+j)*testGrid[2]+k] = double(i)/double(testGrid[0]);
	...
	for ( size_t i = 0 ; i < testGrid[0]; ++i)
		for ( size_t j = 0 ; j < testGrid[1]; ++j)
			for ( size_t k = 0 ; k < testGrid[2]; ++k)
				data[(i*testGrid[1]+j)*testGrid[2]+k] = double(j)/double(testGrid[1]);
	neededData.clear();
	for ( auto i : indices )
		neededData.push_back(data[i]);
	triLin.interpolate(1,neededData,interpolatedData);
	...
	for ( size_t i = 0 ; i < testGrid[0]; ++i)
		for ( size_t j = 0 ; j < testGrid[1]; ++j)
			for ( size_t k = 0 ; k < testGrid[2]; ++k)
				data[(i*testGrid[1]+j)*testGrid[2]+k] = double(k)/double(testGrid[2]);
	neededData.clear();
	for ( auto i : indices )
		neededData.push_back(data[i]);
	triLin.interpolate(1,neededData,interpolatedData);
...
BOOST_AUTO_TEST_CASE( Linear_Interpolation_Blockdata )
{
	...
	const double accuracy = 10e-8;
	//Test that our linear interpolation interpolates
	std::vector<size_t> testGrid = {7,5,2};
	elephon::Algorithms::TrilinearInterpolation triLin(testGrid);
		...
		for ( size_t j = 0 ; j < testGrid[1]; ++j)
			for ( size_t k = 0 ; k < testGrid[2]; ++k)
				for ( size_t ib = 0 ; ib < nBlock; ++ib)
					data[((i*testGrid[1]+j)*testGrid[2]+k)*nBlock + ib] =
							double(ib)+
						double(i)/double(testGrid[0]);
...
BOOST_AUTO_TEST_CASE( Linear_Interpolation_IntegralTest )
{
	...
	std::vector<size_t> testGrid = {60,60,60};
	elephon::Algorithms::TrilinearInterpolation triLin(testGrid);
	...
	for ( size_t i = 0 ; i < testGrid[0]; ++i)
		for ( size_t j = 0 ; j < testGrid[1]; ++j)
			for ( size_t k = 0 ; k < testGrid[2]; ++k)
			{
				...
				data[((i*testGrid[1]+j)*testGrid[2]+k)*nBlock + 0] =
						std::fabs(std::cos(2.0*M_PI*x))*
						std::fabs(std::cos(2.0*M_PI*y))*
						std::fabs(std::cos(2.0*M_PI*z));
				data[((i*testGrid[1]+j)*testGrid[2]+k)*nBlock + 1] =
						std::sin(2.0*M_PI*x)*
						std::sin(2.0*M_PI*y)*
						std::sin(2.0*M_PI*z);
				data[((i*testGrid[1]+j)*testGrid[2]+k)*nBlock + 2] = 1.0;
				if ( (x < 0.5) && (y < 0.5) && (z < 0.5) )
				{
					integralBlock1 += data[((i*testGrid[1]+j)*testGrid[2]+k)*nBlock + 0]*dV;
					integralBlock2 += data[((i*testGrid[1]+j)*testGrid[2]+k)*nBlock + 1]*dV;
					integralBlock3 += data[((i*testGrid[1]+j)*testGrid[2]+k)*nBlock + 2]*dV;
				}
				...
				testPoints[((i*testGrid[1]+j)*testGrid[2]+k)*3 + 0 ] = x/2.0;
				testPoints[((i*testGrid[1]+j)*testGrid[2]+k)*3 + 1 ] = y/2.0;
				testPoints[((i*testGrid[1]+j)*testGrid[2]+k)*3 + 2 ] = z/2.0;
			}
	...
	size_t nPoints = testPoints.size()/3;
